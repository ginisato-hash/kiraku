...
class CleaningGuestInfo:
    ...
    children_age_7plus_count/children_age_known_count/children_age_data_available:
    extract.extract_children_age_7plus()/extract.extract_child_age_info()参照。
    ソースはBeds24 v2の`comments`(2026-09-02、anchor booking 91673623ほか実データ
    6予約7行で確認済み。実在する表記は "N child aged N" の1形のみで、子供が複数の
    場合は1行ずつ別行に現れる)。年齢が判明していない子供の人数は
    children - children_age_known_count で求まる(その分は布団人数へ安全側に加算)。
    ...
    adults+children。
    guest_notice: extract.extract_guest_notice()参照。ソースはBeds24 v2の`comments`
    のみ(内部メモ用のnotes/groupNote/messageとは別経路)。同じ`comments`に混在する
    Booking.comのchild-age metadataとOTA自動生成の定型行(PRE-PAID/BOOKING NOTE/
    部屋設定コード等)を行単位で除去した残りだけを返し、残りが無ければNone。
    ...
    children_age_known_count: int = 0
